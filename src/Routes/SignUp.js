...
class SignUp extends React.Component {
  ...
  nextHandler = () => {
    ...
    else if(this.state.username.length ===0 || this.state.email.length ===0 ||this.state.password.length===0){
  ...
  submitHandler = () => {
    if(this.state.f_name.length===0 || this.state.l_name.length===0 ||this.state.cohort_name==="---"){
      debugger
      this.setState({
  ...
  render () {
    return (
      <div className="CredentialDiv">
        ...
        {!this.state.next ? <FirstSignUpForm changeHandler={this.changeHandler} nextHandler={this.nextHandler} state={this.state}/> :null}
        {this.state.next ? <SecondSignUpForm changeHandler={this.changeHandler} backHandler={this.backHandler} imageSubmit={this.imageSubmit} state={this.state} submitHandler={this.submitHandler}/> :null}
      </div>
  )
  }
}
